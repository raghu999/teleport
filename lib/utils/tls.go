...
func TLSConfig(cipherSuites []uint16) *tls.Config {
	...
	// Use the passed in cipher suites. If unspecified use the defaults.
...
// into uint16 IDs.
func CipherSuiteMapping(s string) (uint16, error) {
	c, ok := cipherSuiteMapping[s]
	if !ok {
		return 0, trace.BadParameter("cipher suite not supported: %v", s)
	}
...
var cipherSuiteMapping map[string]uint16 = map[string]uint16{
	"tls-rsa-with-rc4-128-sha":                tls.TLS_RSA_WITH_RC4_128_SHA,
	"tls-rsa-with-3des-ede-cbc-sha":           tls.TLS_RSA_WITH_3DES_EDE_CBC_SHA,
	...
	"tls-ecdhe-ecdsa-with-rc4-128-sha":        tls.TLS_ECDHE_ECDSA_WITH_RC4_128_SHA,
	...
	"tls-ecdhe-rsa-with-rc4-128-sha":          tls.TLS_ECDHE_RSA_WITH_RC4_128_SHA,
	"tls-ecdhe-rsa-with-3des-ede-cbc-sha":     tls.TLS_ECDHE_RSA_WITH_3DES_EDE_CBC_SHA,
